...
abstract class AbstractObjectDB {
	...
	public static function getAll($class, $table, $count = false, $offset = false) {
		return self::getAllWithOrder($table, $class, "id", true, $count, $offset);
	...
	public static function getAllOnFields($table_name, $class, $field_1, $value_1, $field_2, $value_2, $order = false, $ask = true, $count = false, $offset = false) {
		return self::getOnFields($table_name, $class, "`$field_1` = ".self::$db->getSQ(), array($value_1), "`$field_2` = ".self::$db->getSQ(), array($value_2), $order, $ask, $count, $offset);

	}

	protected static function getOnFields($table_name, $class, $where_1 = false, $values_1 = false, $where_2 = false, $values_2 = false, $order = false, $ask = true, $count = false, $offset = false) {
		$select = new Select();
		$select->from($table_name, "*");
		if ($where_1) $select->where($where_1, $values_1);
		if ($where_2) $select->where($where_2, $values_2);
	...
	public static function getIDsOnField($table_name, $class, $field, $value, $order = false, $ask = true, $count = false, $offset = false) {
		$select = new Select();
	...
	protected function add($field, $type = null, $default = null) {
		$this->properties[$field] = array("value" => $default, "type" => in_array($type, self::$types)? $type : null);
	}
